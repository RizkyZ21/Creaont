// PATH: creaont_flutter/lib/screens/chat/chat_room_screen.dart
...
class ChatRoomScreen extends StatefulWidget {
  ...
  final String token;
  /// myRole masih dipertahankan untuk backward-compat (dipakai saat kirim pesan ke backend),
  /// tapi penentuan "isMe" di bubble chat sekarang pakai myUserId bukan string role.
  final String myRole;
  final int myUserId;
...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  List<int> get _allOrderIds =>
      widget.groupOrderIds.isNotEmpty ? widget.groupOrderIds : [widget.orderId];
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    for (final id in _allOrderIds) {
      final res = await ChatService.getMessages(token: widget.token, orderId: id);
      if (res['success'] == true) {
  ...
  /// FIXED: Tentukan apakah pesan ini milik saya berdasarkan sender.id == myUserId.
  /// Sebelumnya pakai sender_type == myRole yang salah ketika desainer jadi customer
  /// (keduanya punya sender_type 'designer' / 'customer' yang bisa bentrok).
  bool _isMyMessage(dynamic msg) {
    // Prioritas 1: cek sender.id langsung
    final senderId = msg['sender']?['id'];
    if (senderId != null) return senderId == widget.myUserId;

    // Fallback: cek sender_type + myRole (perilaku lama)
    return msg['sender_type'] == widget.myRole;
  }
        ...
        children: [
          if (isGroup)
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.layers_outlined, color: Colors.lightBlueAccent, size: 14),
                  const SizedBox(width: 6),
                  Text('Chat digabung dari ${_allOrderIds.length} order',
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    gradient: isMe
                                        ...
                                        : null,
                                    color: isMe ? null : const Color(0xFF0D1F3C),
